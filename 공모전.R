...
setwd('C:/Users/jaho9/Desktop/공모전/☆ 프로농구 데이터')

########## install.packages ####
install.packages('readxl')
install.packages('randomForest')
isntall.packages('caret')
...
########## Read Data ##########
library(readxl)
d1 <- readxl::read_excel("2016-2017시즌_경기기록데이터.xlsx",
...
d2 <- readxl::read_excel(path = "2017-2018시즌_경기기록데이터.xlsx",
                         sheet = "PLAYER_DAILY_LIST",
                         col_names = T)
d2
summary(d2)
...
##### idf 제거
idf0 <- which(colnames(d)=='idf')
d <- d[,-idf0]

##### foul_tot 제거
foul0 <- which(colnames(d)=='foul_tot')
d <- d[,-foul0]
...
rffactidx <- which(colnames(d23)=='season_code' |colnames(d23)=='game_no'|colnames(d23)=='player_no'|colnames(d23)=='away_team'|colnames(d23)=='playtime'| 
                   colnames(d23)=='fg_r'|colnames(d23)=='ft_r'|colnames(d23)=='threep_r'|colnames(d23)=='dk_r'|colnames(d23)=='pp_r')
library(randomForest)
...
rfidx10<-which(colname=='game_no'|colname=='player_no'|colname=='away_team'|colname=='play_min'|
                 colname=='play_sec'|colname=='fg_r'|colname=='ft_r'|colname=='threep_r'|colname=='dk_r'|colname=='pp_r')
ranfo10<-train_newdata[,-rfidx10]
all_rf10<-randomForest(pos~., data=ranfo10, ntree=500)
all_rf10
plot(all_rf10)

varImpPlot(all_rf10)
varImpPlot(all_rf10)[,1]

y_hat_rf10<-predict(all_rf10,validation,type='prob')
y_hat_rf10<-predict(all_rf10,validation,type='response')
sum(y_hat_rf10==ry_obs)/n1   # 예측율 87.07748
confusionMatrix(y_hat_rf10, ry_obs)


library(MLmetrics)
F1_Score(ry_obs, y_hat_rf10, positive = NULL)
...
pcaidx<-which(colnames(d)=='season_code' | colnames(d)=='game_no'| colnames(d)=='player_no'| colnames(d)=='away_team'
              |colnames(d)=='team_code' | colnames(d)=='home_away' | colnames(d)=='pos' | colnames(d)=='start_flag' | colnames(d)=='play_min' 
              | colnames(d)=='play_sec' | colnames(d)=='fg' | colnames(d)=="fg_a" | colnames(d)=="ft" | colnames(d)=="ft_a"| colnames(d)=="threep"
              | colnames(d)=="threep_a"| colnames(d)=="dk"| colnames(d)== "dk_a"| colnames(d)=="pp"| colnames(d)=="pp_a" | colnames(d)=="score")
...
rownames(V2all)<-colnames(d[,-pcaidx])
V2all
...
newidx<-which(colnames(d)=='season_code' |colnames(d)=='game_no'| colnames(d)=='team_code' | colnames(d)=='home_away' | colnames(d)=='player_no'|
                colnames(d)=='pos' |colnames(d)=='away_team'|colnames(d)=='start_flag' |colnames(d)=='play_min' |
                 colnames(d)=='play_sec'|colnames(d)=='fg_r'|colnames(d)=='ft_r'|colnames(d)=='threep_r'|colnames(d)=='dk_r'|colnames(d)=='pp_r' )
dnew<-d[,-newidx]
...
str(mean_gd)
dim(mean_gd)
dgd_racarchart <- function(dgdint){
...
windCidx <- which(colnames(dc)=='game_no' | colnames(dc)=='player_no' | colnames(dc)=='away_team' |
			...
			colnames(dc)=='pp_a' | colnames(dc)=='pos')
...
windFDidx <- which(colnames(dfd)=='game_no' | colnames(dfd)=='player_no' | colnames(dfd)=='away_team' |
			...
			colnames(dfd)=='pp_a' | colnames(dc)=='pos')
...
windGDidx <- which(colnames(dgd)=='game_no' | colnames(dgd)=='player_no' | colnames(dgd)=='away_team' |
			...
			colnames(dgd)=='pp_a' | colnames(dc)=='pos')
...
dim(playercar)[1]
dim(playerc)[1] + dim(playerfd)[1] + dim(playergd)[1]
...
# 290750 - 290450 - 290781 - 290987 - 290549 - 290248 - 290110 - 290439 - 290491 - 
# 290786
code_C=c('290750', '290450', '290781', '290987', '290549', '290248', '290110',
	'290439', '290491', '290786')
C_10=rep(0,11)
for (i in 1:length(code_C)){
  C_10=rbind(C_10,player_code[which(player_code$player_no==code_C[i]),])
}

#291107 290395 291180 291171 291011 291108 290978 
#291058 290981 290367 290774 291132 
...
# 290787 - 290897 - 291091 - 290417 - 291084 - 290993 - 290280 - 290119 - 290661 - 
# 290284

code_FD=c('290787', '290897', '291091', '290417', '291084', '290993', '290280',
	'290119', '290661', '290284')
FD_10=rep(0,11)
...
# 290381 - 290440 - 290776 - 291085 - 290742 - 290407 - 290505 - 210074 - 290991 - 
# 291008
...
ze <- which(ALL_30$player_no==0)
ALL_30 <- ALL_30[-ze,]
ALL_30 <- ALL_30[-c(15,25,28),]
length(ALL_30[,2])

remove=c('291107', '290395' ,'291180', '291171', '291011', '291108' ,'290978' ,'291058' ,'290981', '290367', '290774', '291132', '291133', '291055', '291139', '290861', '291137', '291014', '291134', '291129' ,'290371',
         '290982', '291140' ,'290325', '290261' ,'291138' ,'291052' ,'291057' ,'290524' ,'230080', '290738', '291145', '290993', '291109', '291233' ,'290984', '291172' ,'290983', '290742', '290412' ,'290794', '291144' ,
         '290381', '290797', '290280', '290750', '290542', '290505' ,'291220' ,'290976' ,'290106', '291056' ,'215053', '291234' ,'290790', '291136' ,'290485' ,'290862' ,'290407' ,'290765', '290558' ,'290909', '290740', 
         '235070', '290450', '290553', '291015', '290227' )
length(remove)
...
finalC0 <- ALL_30[ALL_30$pos=="C",]
finalFD0 <- ALL_30[ALL_30$pos=="FD",]
finalGD0 <- ALL_30[ALL_30$pos=="GD",]
...
dim(finalC0)[1];dim(finalGD0)[1];dim(finalFD0)[1]

finalC <- finalC0[1:4,3]
finalFD <- finalFD0[1:6,3]
finalGD <- finalGD0[1:6,3]
...
remove_code[1]
remove_code
# GD : 290742, 290381, 290505, 290407, 220194, 290380, 290440, 
# GD : 291085, 290776, 210074, 291008, 290991
# FD : 290993, 290280, 290542, 290790, "290417", "291084", 290897, 
# FD : "291091", 290284, 290119, 291001, 290787, 290100 
# C  : 290750, 290450, 290248, 290781, 290987
...
remove[67]
# C  : 290750 / 290450
# FD : 290993 / 290280 / ★290417
# GD : 290742 / 290381 / 290505 / 290407
...
a <- which(d29$player_no=="290407")
b <- which(d31$player_no=="290407")
c <- which(d33$player_no=="290407")
